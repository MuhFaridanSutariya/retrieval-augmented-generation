...
SAMPLE_FILENAME = "sample_complex.pdf"


def generate_sample_complex_pdf() -> bytes:
    buffer = io.BytesIO()
...
def _build_story() -> list:
    ...
    incidents_data = [
        ...
        ["2026-01-08", "S2", "32 min", "OpenAI rate-limit cascade", "Added per-tenant token bucket"],
        ["2026-01-22", "S3", "11 min", "Stale BM25 index after redeploy", "Generation counter in FaissStore"],
        ["2026-02-09", "S1", "47 min", "Postgres connection-pool exhaustion", "Bumped pool_size from 10 to 25"],
        ["2026-03-17", "S2", "18 min", "Reranker JSON parse failure on emoji input", "Defensive parse fallback to fused order"],
    ]
    incidents_table = Table(
        incidents_data,
        hAlign="LEFT",
        colWidths=[0.9 * inch, 0.6 * inch, 0.7 * inch, 1.8 * inch, 2.0 * inch],
    )
